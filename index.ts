const api = {
  ...
  function displayResults(weather) {
    let city = document.querySelector<HTMLElement>('.location .city');
    city.innerText = `${weather.name}, ${weather.sys.country}`;
  
    let now = new Date();
    let date = document.querySelector<HTMLElement>('.location .date');
    date.innerText = dateBuilder(now);
  
    let temp = document.querySelector<HTMLElement>('.current .temp');
    temp.innerHTML = `${Math.round(weather.main.temp)}<span>°c</span>`;
  
    let weather_el = document.querySelector<HTMLElement>('.current .weather');
    weather_el.innerText = weather.weather[0].main;
  
  
    let hilow = document.querySelector<HTMLElement>('.hi-low');
    hilow.innerText = ` ${Math.round(weather.main.temp_min)}°c / Max: ${Math.round(weather.main.temp_max)}°c`;
  
    let humid = document.querySelector<HTMLElement>('.humidity');
    humid.innerText = `Humidity: ${weather.main.humidity}%`;
  
    let wind = document.querySelector<HTMLElement>('.wind');
  ...
  function displayForecast(data) {
    ...
    forecastData.forEach((dayData, index) => {
      ...
      const dateElement = document.querySelector<HTMLElement>(`.date${index + 1}`);
      const tempElement = document.querySelector<HTMLElement>(`.temp${index + 1}`);
      const weatherElement = document.querySelector<HTMLElement>(`.weather${index + 1}`);
      const hiLowElement = document.querySelector<HTMLElement>(`.hi-low${index + 1}`);
      const humidityElement = document.querySelector<HTMLElement>(`.humidity${index + 1}`);
      const windElement = document.querySelector<HTMLElement>(`.wind${index + 1}`);
